...
-(void) showLoadingWheel {
	[loadingWheel removeFromSuperview];
	self.loadingWheel = [[[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(self.bounds.size.width/2-25, self.bounds.size.height/2-25, 50, 50)] autorelease];
//	loadingWheel.center = self.center;
    
    NSLog(@"width=%f",self.bounds.size.width);
     NSLog(@"heigh=%f",self.bounds.size.height);
    loadingWheel.color = [UIColor greenColor];
